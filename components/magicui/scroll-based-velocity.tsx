...
/** Wrap a value between min..max (positive modulus). */
...
export function ScrollVelocityContainer({
  ...
  // Map raw scroll velocity to a small signed factor [-5..5]
...
function ScrollVelocityRowImpl({
  ...
  useAnimationFrame((_, delta) => {
    ...
    // motion-safe: don’t amplify movement when PRM is set
    const speedMultiplier = prefersReducedMotionRef.current ? 1 : 1 + absVf;

    // flip direction with scroll intent if velocity is meaningful
    if (absVf > 0.1) {
    ...
    const pixelsPerSecond = (bw * baseVelocity) / 100; // % of content width per second
    const moveBy =
